...
const WZ_GMS_IV = [0x4D, 0x23, 0xC7, 0x2B] as const;

async function createCipher(userKey: readonly number[], userIv: readonly number[], size: number): Promise<Uint8Array> {
...
let cipher = await createCipher(AES_USER_KEY, WZ_GMS_IV, 0x2000); // TODO make user key, iv configurable
